...
do
  ...
  do
    ...
    do
      rate_pruned_stbs=`grep pruned_slices out_stbs_$u\_$exp\_$delta\_$run.txt | cut -d ' ' -f3`
  ...
  do
    for((run=1; run<=$num_runs; run++))
    do
      rate_pruned_stus=`grep pruned_slices out_stus_$u\_$exp\_$delta\_$run.txt | cut -d ' ' -f3`
