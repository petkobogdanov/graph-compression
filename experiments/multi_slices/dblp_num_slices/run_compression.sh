...
do
  ...
  do
    ...
    do
		...
		#echo "$graph_compression -c STBS -x -p $p -m $max_radius -g $data/dblp.graph -s $data/dblp.index -v $data/$g.data -n $rate_slow_sampling -d $delta_slow_sampling -r $rho_slow_sampling > out_stbs_${g}_${p}_${r}"
		#$graph_compression -c STBS -x -p $p -m $max_radius -g $data/dblp.graph -s $data/dblp.index -v $data/$g.data -n $rate_slow_sampling -d $delta_slow_sampling -r $rho_slow_sampling > out_stbs_${g}_${p}_${r}
