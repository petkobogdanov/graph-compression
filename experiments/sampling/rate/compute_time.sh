...
rm $results_time\_stus_fast.dat
...
do
  ...
  do
	...
	ctime=`grep compression_time out_stus_fast_$postfix_samp.txt | cut -d ' ' -f3`
        avg_stus_fast=`echo "scale=10; $avg_stus_fast+$ctime" | bc`
  ...
  echo "$p	$avg_stus_fast" >> $results_time\_stus_fast.dat
  echo "$p	$avg_stus_slow" >> $results_time\_stus_slow.dat
done
  
avg_st=`echo "scale=10; $avg_st/($num_graphs*${#param_rate[@]})" | bc`
