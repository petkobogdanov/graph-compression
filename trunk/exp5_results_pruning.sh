...
do
  ...
  do
    ...
    do
      rate_pruned_stbs=`grep pruned_slices out_stbs_$c\_$exp\_$run.txt | cut -d ' ' -f3`
      rate_pruned_stbs=`echo ${rate_pruned_stbs} | sed -e 's/[eE]+*/\\*10\\^/'`
      count_1=`grep count_bound_1 out_stbs_$c\_$exp\_$run.txt | cut -d ' ' -f3`
      count_2=`grep count_bound_2 out_stbs_$c\_$exp\_$run.txt | cut -d ' ' -f3`
      count_3=`grep count_bound_3 out_stbs_$c\_$exp\_$run.txt | cut -d ' ' -f3`
  ...
  do
    for((run=1; run<=$num_runs; run++))
    do
      rate_pruned_stus=`grep pruned_slices out_stus_$c\_$exp\_$run.txt | cut -d ' ' -f3`
      rate_pruned_stus=`echo ${rate_pruned_stus} | sed -e 's/[eE]+*/\\*10\\^/'`
      count_1=`grep count_bound_1 out_stus_$c\_$exp\_$run.txt | cut -d ' ' -f3`
      count_2=`grep count_bound_2 out_stus_$c\_$exp\_$run.txt | cut -d ' ' -f3`
      count_3=`grep count_bound_3 out_stus_$c\_$exp\_$run.txt | cut -d ' ' -f3`
      rate_1=`echo "scale=10; ($count_1+$count_2)/($count_1+$count_2+$count_3)" | bc`
      rate_2=`echo "scale=10; $count_3/($count_1+$count_2+$count_3)" | bc`
